...
interface MemberRow {
  id: number
  name: string
  category: keyof typeof MemberCategoryLabels
  phone: string
  storeId: number
...
const filters = reactive({ keyword: '', category: '', storeId: undefined as number | undefined, channel: '' })
...
function openCreate() {
  ...
  Object.assign(form, {
    name: '', category: 'NORMAL', phone: '', storeId: undefined,
    channel: 'WALK_IN', coachId: null, preferenceTags: [], remark: '',
  })
...
async function handleSave() {
  if (!form.name || !form.phone || !form.storeId) {
    ElMessage.warning('请填写姓名、手机号并选择门店')
    return
  }
  saving.value = true
  try {
...
<template>
  <el-card shadow="never">
    ...
    <div class="toolbar">
      ...
      <el-select v-model="filters.category" placeholder="顾客分类" clearable class="w-140">
        <el-option v-for="(label, key) in MemberCategoryLabels" :key="key" :label="label" :value="key" />
      </el-select>
    ...
    <el-table v-loading="loading" :data="items" border stripe>
      ...
      <el-table-column label="顾客分类" width="100">
        <template #default="{ row }">
          <el-tag :type="row.category === 'SVIP' ? 'danger' : row.category === 'VIP' ? 'warning' : 'info'">
            {{ MemberCategoryLabels[asRow(row).category] }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="phone" label="手机号" width="130" />
      <el-table-column label="门店名称" min-width="120">
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑会员' : '新增会员'" width="520px">
      <el-form label-width="90px">
        ...
        <el-form-item label="教练">
          <el-select v-model="form.coachId" placeholder="可不选" clearable>
