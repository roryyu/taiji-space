...
「太极空间」是一套面向太极/健身场馆的会员课程管理系统，为管理员提供会员全生命周期管理、会员卡资产管理、课程创建/排期/预约/评价、员工管理、运营数据分析与系统配置能力。
...
Staff 1─n Member(教练)   Staff 1─n Course
...
- **功能**：填删改查（员工列表）。
- **字段**：姓名、资质、风格标签（多标签）、账号、密码、类型（教师/管理员）。
- **删除**：员工名下存在课程时禁止删除。
...
| 会员 | 标签 | 课程偏好标签出现频次分布 |
| 员工 | 课程完成率 | 该员工排期中 FINISHED / 总排期（剔除 CANCELLED） |
| 员工 | 学员评价统计 | 名下课程评价均分与评价数 |
...
| 员工 | GET/POST /api/teachers、PUT/DELETE /api/teachers/:id | 员工 CRUD |
| 分析 | GET /api/analytics/overview | 总览卡片 |
| | GET /api/analytics/members | 会员维度 |
| | GET /api/analytics/teachers | 员工维度 |
